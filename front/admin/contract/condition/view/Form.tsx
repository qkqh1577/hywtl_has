...
import {
  ...
  initialContractConditionVOFromServer
...
export default function Form({ variableList }: Props) {
  ...
  const onAddForm = () => {
    formikContext.setFieldValue(`contractConditionList`, [...(list ?? []), initialContractConditionVOFromServer]);
  };
                  ...
                  <Tooltip title="순서 올리기">
                    <IconButton
                      ...
                      onClick={() => {
                        ...
                        const contractConditionList: ContractConditionVOFromServer[] = [];
                        for (let k = 0; k < prevList.length; k++) {
